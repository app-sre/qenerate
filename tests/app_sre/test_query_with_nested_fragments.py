...
def get_gql_file(name: str) -> str:
    with open(f"tests/app_sre/gql/query_with_nested_fragments/{name}.gql") as f:
        return f.read()


def get_expected(name: str) -> str:
    with open(
        f"tests/app_sre/gql/query_with_nested_fragments/expected/pydantic_v1/{name}.py.txt"
    ) as f:
        return f.read()
